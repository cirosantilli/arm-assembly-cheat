...
For other operating systems, see: <<getting-started-non-ubuntu>>.

We compile our own Binutils and QEMU to be able to use the newest ISA features. Those projects build pretty fast (~10 minutes), so it is fine.

The cleanest thing would be to also compile GCC with link:https://github.com/crosstool-ng/crosstool-ng[crosstool-NG], but the build is much longer, and I'm lazy, let's just wait until something fails in obscure ways.
...
Alternatively, to help with tab complete, the following shortcuts all do the same thing as `make test-add`:
...
....
+
then inside Docker just add the `--docker` flag to `./download-dependencies`:
+
....
...
For non-Linux systems, just use an Ubuntu virtual machine such as VirtualBox: link:https://askubuntu.com/questions/142549/how-to-install-ubuntu-on-virtualbox[].
...
* link:v7/inline/reg_var.c[]
* link:v8/inline/reg_var.c[]
* link:v8/inline/reg_var_float.c[]
...
<<armarmv8>> F2.9 "Additional information about Advanced SIMD and floating-point instructions" says:

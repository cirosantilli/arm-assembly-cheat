...
ARMv7 and ARMv8 assembly userland minimal examples tutorial. Runnable asserts on x86 host with QEMU user mode or natively on an ARM guest. Nice GDB step debug setup. Tested on Ubuntu 18.04 host and Raspberry Pi 2 guest.
...
This leaves us right at the end of the prolog of `asm_main`, and at the start of the assembly code in the `.S` file.
...
GDB TUI is slightly buggier on the ancient 4.9 toolchain (current line gets different indentation, does not break on the right instruction after `asm_main_after_prolog`, link:https://superuser.com/questions/180512/how-to-turn-off-gdb-tui[cannot leave TUI]), but it might still be usable

I believe that the exact current setup will work on the Raspberry Pi 3, which has a link:https://www.raspberrypi.org/documentation/hardware/raspberrypi/bcm2837/README.md[BCM2837] SoC, which has 4 link:https://en.wikipedia.org/wiki/ARM_Cortex-A53[Cortex A53] cores, which implement ARMv8-A, so you just have to go to the ARMv8 folder instead. TODO test it.

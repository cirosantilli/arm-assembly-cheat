...
Almost all the content in this repository has been moved to: https://github.com/cirosantilli/linux-kernel-module-cheat#arm-assembly only the core infrastructure was left here.

Notable advantages of LKMC repository include:

* a single unified cross arch setup for ARM and x86_64
...
There is only one use case left for this repository: since this is more minimal, it is easier to upgrate to the latest binutils-gdb here, in order to get very new instructions.

The initial motivation for that was link:v8/sve.S[], although that specific case will soon be covered on LKMC as well most likely.

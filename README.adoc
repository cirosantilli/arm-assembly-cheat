...
Almost all the content in this repository has been moved to: https://github.com/cirosantilli/linux-kernel-module-cheat#arm-assembly
...
It is very confusing, but `fadds` and `faddd` in ARMv7 and Aarch32 are <<gnu-gas-assembler-arm-unified-syntax,pre-UAL>> for `vadd.f32` and `vadd.f64`.
...
* link:userland/arch/aarch64/floating_point.S[]
...
Example: link:userland/arch/aarch64/simd_interleave.S[]
...
Non-formal introductory tutorials are extremely scarce.

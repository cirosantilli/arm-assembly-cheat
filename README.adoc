...
Added in ARMv8 <<aarch32>> only, not present in ARMv7, likely because different rounding modes were not available back then.

Very similar to <<vcvt>>, but:

* can only do float to int
* rounds to nearest with Ties to Away (from 0.0): e.g. `1.5` ties, so it goes away from `0` to `2`
